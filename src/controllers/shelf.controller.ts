...
export const handleListAllShelf = async (
  request: FastifyRequest<{ Params: { id_lib: string }}>,

  reply: FastifyReply
) => {
  const {id_lib} = request.params;
  //const filters = request.query;

  if (id_lib !== null && id_lib !== "") {
    const result = await shelfService.findAll(id_lib);

    const checkLib = await libService.findById(id_lib);

    if (!checkLib) {
      return reply.status(400).send({ message: "Library invalid!" });
    }
  ...
  return reply.status(400).send({ message: "[Library_ID] parameter is mandatory" });
};
